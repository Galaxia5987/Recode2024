...
import frc.robot.subsystems.leds.READY_TO_SHOOT_COLOR
...
class ShootState : ScoreState {
    ...
    fun init(): Command {
        return Commands.parallel(
            warmup(),
            turnToSpeaker(),
            leds.setSolidMode(READY_TO_SHOOT_COLOR).onlyIf(::readyToShoot)
        )
    }
    ...
    fun end(): Command {
        return gripper.feed().andThen(
            Commands.parallel(
                ...
                hood.setRestingAngle(), leds.setSolidMode(SHOOT_STATE_COLOR)
            )
        )
    }
